...
## }}}
hdpGLM_simulateData <- function(n, K, nCov, nCovj=0, J=1, parameters=NULL, pi=NULL, family, seed=sample(1:777,1))
{
    ## error handling
    ...
    if (is.null(parameters)) {
        parameters <- hdpGLM_simulateParameters(K=K, nCov=nCov, nCovj=nCovj, J=J, pi, seed=seed) 
    }
...
#' @export
## }}}
hdpGLM_simulateParameters <- function(nCov, K, nCovj=0, J=1, pi=NULL, seed=NULL)
{
    ...
    for (j in 1:J) {
        for (k in 1:K) {
            parameters$beta[[j]][[k]] = MASS::mvrnorm(n=1, mu=W[j, ] %*% tau, .01*diag(nCov+1))
