...
#' @param plot.cluster.freq boolean, if \code{TRUE} it generates a bar plot shows the clusters and how frequent data points were classified in that cluster during the estimation of the model
...
## }}}
plot.dpGLM    <- function(x, K=1, separate=F, hpd=T, true_parameters=NULL, plot.hist=T, plot.cluster.freq=T, layout=NULL, title=NULL, prop.time.active=.95, adjust=.8, ...)
{
    ...
    }else
    {
        ...
        for (d in 1:ncol(beta))
        {
            ...
            if(!is.null(true_parameters)) graphics::legend('topright', legend=paste0('True in Cluster ',1:K, sep=''), col=cols, lty=1:K, bty='n', cex=.9)
        }
    }
