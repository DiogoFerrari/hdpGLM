...
arma::mat hdpGLM_get_theta_active(arma::mat theta, arma::colvec Z)
{
...
arma::mat hdpGLM_get_inits(int K,int d, String family, List fix)
{
  arma::mat theta(K, 1); 
...
// Theta
// -----
arma::mat hdpGLM_update_theta(arma::colvec y, arma::mat X, arma::colvec Z, int K, arma::mat theta, List fix, String family, double epsilon, int leapFrog, int hmc_iter)
{
  if(family == "gaussian"){
...
List hdpGLM_mcmc(arma::colvec y, arma::mat X, arma::mat W, arma::colvec C, arma::colvec weights, int K, List fix, String family, List mcmc, double epsilon, int leapFrog, int n_display, int hmc_iter)
{
  ...
  arma::mat theta = hdpGLM_get_inits(K, d, family, fix);
  ...
  for(int iter = 0; iter < N; iter++){
    ...
    Z	     = hdpGLM_update_Z(y, X, pi, K, theta, family);
    theta    = hdpGLM_update_theta(y, X, Z, K, theta,  fix, family, epsilon, leapFrog, hmc_iter);

    // saving samples
    // --------------
    if(iter+1 > burn_in){
      arma::mat theta_new = hdpGLM_get_theta_active(theta, Z);
